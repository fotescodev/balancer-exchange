...
async function generate(lists, data, metadata) {
    ...
    await generateNetwork('moonbase', lists, data, metadata);
}
...
async function getMetadata(tokens, overwrite) {
    ...
    const moonbase = await getNetworkMetadata(
        'moonbase',
        tokens.moonbase,
        overwrite.moonbase
    );
...
async function getNetworkMetadata(network, tokens, overwrite) {
    ...
    const providers = {
        ...
        moonbase: new ethers.providers.JsonRpcProvider(
            process.env.REACT_APP_RPC_URL_1287
        ),
    };
    ...
    const multicallContract = {
        ...
        moonbase: process.env.CONTRACT_MULTICALL,
    };
...
function mergeTokenLists(lists) {
    ...
    for (const datasetName in lists) {
        ...
        let dataset_moonbase = [];
        if (dataset.moonbase instanceof Array) {
            dataset_moonbase = dataset.moonbase;
        } else {
            dataset_moonbase = Object.keys(dataset.moonbase);
        }
        ...
        for (const token of dataset_moonbase) {
            moonbase.push(token);
        }
    }
...
function verifyInputs(lists) {
    ...
    verifyNetworkInputs(lists, 'moonbase');
