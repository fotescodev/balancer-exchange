...
async function getPriceByAddresses(coinIds) {
    ...
    for (let i = 0; i < addresses.length / ITEMS_PER_BATCH; i++) {
        const addressSlice = addresses.slice(
            ITEMS_PER_BATCH * i,
            ITEMS_PER_BATCH * (i + 1)
        );
        const contractAddressesString = addressSlice.join(',');
        const priceResponse = await coingeckoClient.get(
...
async function getPriceBySymbols(coinIds) {
    ...
    for (let i = 0; i < addresses.length / ITEMS_PER_BATCH; i++) {
        ...
        const idsString = addressSlice
            .map(address => coinIds[address])
            .join(',');
        ...
        for (const address of addressSlice) {
            ...
            prices[address] = priceInfo ? priceInfo.usd : undefined;
        }
    }
    return prices;
}
...
function comparePrices(coinIds, addressPrices, symbolPrices) {
    for (const address in coinIds) {
        ...
        if (diff > 0.01) {
            console.log(
                `Price mismatch for ${address}: ${addressPrice} vs ${symbolPrice}`
            );
        }
    }
}
